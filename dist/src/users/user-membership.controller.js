...
export const updateMembership = async (req, res) => {
    try {
        ...
        const update = {};
        if (status !== undefined)
            update['membership.status'] = status;
        if (monthlyFee !== undefined)
            update['membership.monthlyFee'] = monthlyFee;
        if (currency !== undefined)
            update['membership.currency'] = currency;
        if (startDate !== undefined)
            update['membership.startDate'] = startDate;
        if (nextDueDate !== undefined)
            update['membership.nextDueDate'] = nextDueDate;
...
export const getMembershipPayments = async (req, res) => {
    try {
        const { id } = req.params;
        if (!isValidObjectId(id)) {
            res.status(400).json({ error: 'ID de usuario no válido' });
...
export const addMembershipPayment = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const user = await User.findByIdAndUpdate(id, { $push: { membershipPayments: req.body } }, { new: true, runValidators: true }).select('-password');
        if (!user) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
...
export const updateMembershipPayment = async (req, res) => {
    try {
        ...
        const update = {};
        if (period !== undefined)
            update['membershipPayments.$.period'] = period;
        if (amount !== undefined)
            update['membershipPayments.$.amount'] = amount;
        if (currency !== undefined)
            update['membershipPayments.$.currency'] = currency;
        if (status !== undefined)
            update['membershipPayments.$.status'] = status;
        if (dueDate !== undefined)
            update['membershipPayments.$.dueDate'] = dueDate;
        if (paidAt !== undefined)
            update['membershipPayments.$.paidAt'] = paidAt;
        if (paymentMethod !== undefined)
            update['membershipPayments.$.paymentMethod'] = paymentMethod;
        if (notes !== undefined)
            update['membershipPayments.$.notes'] = notes;
        if (Object.keys(update).length === 0) {
            res.status(400).json({ error: 'No hay campos para actualizar' });
            return;
        }
        const user = await User.findOneAndUpdate({ _id: id, 'membershipPayments._id': paymentId }, { $set: update }, { new: true, runValidators: true }).select('-password');
        if (!user) {
...
export const removeMembershipPayment = async (req, res) => {
    try {
        const { id, paymentId } = req.params;
        if (!isValidObjectId(id) || !isValidObjectId(paymentId)) {
            res.status(400).json({ error: 'ID no válido' });
        ...
        const user = await User.findByIdAndUpdate(id, { $pull: { membershipPayments: { _id: paymentId } } }, { new: true, runValidators: true }).select('-password');
        if (!user) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
