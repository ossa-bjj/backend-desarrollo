...
const isOwnerOrAdmin = (req, userId) => typeof userId === 'string' && (req.user?.id === userId || req.user?.rol === UserRole.ADMIN);
...
export const updateCustomer = async (req, res) => {
    try {
        ...
        if (isCustomer !== undefined)
            update['customer.isCustomer'] = isCustomer;
        if (origin !== undefined)
            update['customer.origin'] = origin;
        if (since !== undefined)
            update['customer.since'] = since;
...
export const updateSportsProfile = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (isAthlete !== undefined)
            update['sportsProfile.isAthlete'] = isAthlete;
        if (isFederated !== undefined)
            update['sportsProfile.isFederated'] = isFederated;
        if (licenseNumber !== undefined)
            update['sportsProfile.licenseNumber'] = licenseNumber;
        if (federationName !== undefined)
            update['sportsProfile.federationName'] = federationName;
        if (clubName !== undefined)
            update['sportsProfile.clubName'] = clubName;
        if (Object.keys(update).length === 0) {
            res.status(400).json({ error: 'No hay campos para actualizar' });
            return;
        }
...
export const removeSportsProfile = async (req, res) => {
    try {
        const { id } = req.params;
        if (!isValidObjectId(id)) {
            res.status(400).json({ error: 'ID de usuario no válido' });
        ...
        const user = await User.findByIdAndUpdate(id, { $unset: { sportsProfile: '' } }, { new: true }).select('-password');
        if (!user) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
...
export const addAddress = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(id, { $push: { 'profile.addresses': req.body } }, { new: true, runValidators: true }).select('-password');
        if (!user) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
...
export const updateAddress = async (req, res) => {
    try {
        ...
        const update = {};
        if (calle !== undefined)
            update['profile.addresses.$.calle'] = calle;
        if (ciudad !== undefined)
            update['profile.addresses.$.ciudad'] = ciudad;
        if (provincia !== undefined)
            update['profile.addresses.$.provincia'] = provincia;
        if (codigoPostal !== undefined)
            update['profile.addresses.$.codigoPostal'] = codigoPostal;
        if (pais !== undefined)
            update['profile.addresses.$.pais'] = pais;
        if (esPredeterminada !== undefined)
            update['profile.addresses.$.esPredeterminada'] = esPredeterminada;
        if (Object.keys(update).length === 0) {
            res.status(400).json({ error: 'No hay campos para actualizar' });
            return;
        }
        const user = await User.findOneAndUpdate({ _id: id, 'profile.addresses._id': addressId }, { $set: update }, { new: true, runValidators: true }).select('-password');
        if (!user) {
...
export const removeAddress = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(id, { $pull: { 'profile.addresses': { _id: addressId } } }, { new: true, runValidators: true }).select('-password');
        if (!user) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
