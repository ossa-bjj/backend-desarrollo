...
export const createUser = async (req, res) => {
    try {
        const { username, email, password, role, status, profile, customer, sportsProfile, membership, membershipPayments, metadata, } = req.body;
...
export const updateUser = async (req, res) => {
    try {
        const { id } = req.params;
        const { username, email, role, status, profile, customer, sportsProfile, membership, membershipPayments, metadata, } = req.body;
        if (!isValidObjectId(id)) {
            res.status(400).json({ error: 'ID de usuario no válido' });
            return;
        }
        ...
        const hasAdminFields = role !== undefined ||
            status !== undefined ||
        ...
        if (sportsProfile !== undefined)
            update.sportsProfile = sportsProfile;
        if (membership !== undefined)
            update.membership = membership;
        if (membershipPayments !== undefined)
            update.membershipPayments = membershipPayments;
