...
const isAdmin = (req) => req.user?.rol === UserRole.ADMIN;
const canAccessOrder = (req, userId) => isAdmin(req) || String(userId?._id ?? userId) === req.user?.id;
// GET /api/pedidos
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const order = await Order.findByIdAndUpdate(id, { status }, { new: true, runValidators: true }).populate('user', 'username email');
        if (!order) {
            res.status(404).json({ error: 'Pedido no encontrado' });
            return;
        }
